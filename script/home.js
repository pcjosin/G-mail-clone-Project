...
async function clickHandle(emailElementId) {
  fetch("mail.html")
    ...
    .then((data) => {
      ...
      emailSenderText = document.getElementById("sender-adress");
      emailSenderTime = document.getElementById("send-date")
    })
    .catch((error) => console.error("Error:", error));

  emailSubjectContent = await getSendSubject(emailElementId);
  emailBodyContent = await getEmailBodyHtml(emailElementId);
  emailSendTime = await getSendTime(emailElementId);
  emailSender = await getSenderEmail(emailElementId);
  ...
  emailSenderText.innerHTML = emailSender;
  emailSenderTime.innerHTML = emailSendTime;
